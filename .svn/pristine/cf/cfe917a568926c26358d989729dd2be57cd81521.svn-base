...
var imagePuzzle = {
    ...
    setImage: function (images, gridSize = 4) {
        ...
        for (var i = 0; i < gridSize * gridSize; i++) {
            ...
            li.style.width = (360 / gridSize) + 'px';
            li.style.height = (360 / gridSize) + 'px';

            li.onclick = (event) => {
                console.log(i + "<>" + event.target.id);
                li.style.border = "2px solid white";
                li.style.width = ((360 / gridSize) - 2) + 'px';
                li.style.height = ((360 / gridSize) - 2) + 'px';
                li.style.opacity = 0.2;
                let clickChoose = sessionStorage.getItem("clickChoose");
                if (clickChoose == null || clickChoose == "undefined" || clickChoose == "null" || clickChoose == undefined) {
            ...
            li.ondrop = (event) => {
                ...
                let dest = helper.doc(event.target.id);
                let p = dest.parentNode;
                console.log("end ondrop=" + event.dataTransfer.getData('data') + " target_id=" + event.target.id);
                if (origin && dest && p) {
                    let temp = dest.nextSibling;
                    let x_diff = origin.offsetLeft - dest.offsetLeft;
                    let y_diff = origin.offsetTop - dest.offsetTop;
